...
#    plotAX3.set_yscale('log')
    ...
    plt.plot( zipj(data_c2, "ts"), zipj(data_c2, "ACK_coming_speed"), "-", label="ACK_coming_speed2")
    plt.legend()
    
    DNAME='my_max_send_q'
    plotAX3 = plt.subplot(512)
    plt.title(DNAME + " (client)")
    plt.plot(zipj(data_c1, "ts"), zipj(data_c1, DNAME), "-", label="my_max_send_q1", color="b")
    plt.plot(zipj(data_c2, "ts"), zipj(data_c2, DNAME), "-", label="my_max_send_q2", color="g")
    plt.plot(zipj(data_c1, "ts"), zipj(data_c1, "send_q_limit"), "-", label="send_q_limit1", color="r")
    plt.plot(zipj(data_c2, "ts"), zipj(data_c2, "send_q_limit"), "-", label="send_q_limit2", color="k")
    plt.legend()
    
    DNAME='rtt'
    plotAX3 = plt.subplot(513)
    plt.title("rtt (client)")
    plt.plot(zipj(data_c1, "ts"), zipj(data_c1, DNAME), "-", label="rtt1", color="b")
    plt.plot(zipj(data_c2, "ts"), zipj(data_c2, DNAME), "-", label="rtt2", color="g")
    plt.plot(zipj(data_c1, "ts"), zipj(data_c1, 'my_rtt'), "-", label="my_rtt1", color="r")
    plt.plot(zipj(data_c2, "ts"), zipj(data_c2, 'my_rtt'), "-", label="my_rtt2", color="k")
    plt.plot(zipj(data_c1, "ts"), zipj(data_c1, 'magic_rtt'), "-", label="magic_rtt_1", c="m")
    plt.plot(zipj(data_c2, "ts"), zipj(data_c2, 'magic_rtt'), "-", label="magic_rtt_2", c="y")
    plt.legend()

    DNAME="buf_len"    
    plotAX1 = plt.subplot(514)
    plt.title(DNAME+ " (server, client hold)")
    plt.plot(zipj(data_s1, "ts"), zipj(data_s1, DNAME), "-")
    plt.plot(zipj(data_c1, "ts"), numpy.array(zipj(data_c1, "hold_mode"))*90, ".", label="hold_mode1")
    plt.plot(zipj(data_c2, "ts"), numpy.array(zipj(data_c2, "hold_mode"))*100, ".", label="hold_mode2")
    try:
        plt.plot(zipj(data_c1, "ts"), numpy.array(zipj(data_c1, "R_MODE"))*50, ".", label="R_MODE1")
        plt.plot(zipj(data_c2, "ts"), numpy.array(zipj(data_c2, "R_MODE"))*45, ".", label="R_MODE2")
    except KeyError:
